...
const App = () => {
  ...
  //
  const changeValues = (list, item, provided, snapshot) => {
    setSelected({
      ...item,
      list,
    });
    setFormInitialValues((prev) => ({ ...prev, ...item, list }));
    modalOpenHandler();
  };
  ...
  const submitHandler = (values) => {
    const arr =
      values?.list &&
      formValues[values?.list]?.map((item) => {
    ...
    setFormValues((prev) => ({ ...prev, [values?.list]: arr }));
    closeModalHanlder();
  };
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd} enableDefaultSensors>
      <Grid container>
        <Grid item md={8}>
          <Content>
            {Object.keys(formValues).map((list, i) => {
              ...
              return (
                <Droppable key={list} droppableId={list}>
                  {(provided, snapshot) => (
                    ...
                    >
                      <Grid container>
                        {formValues[list].length
                          ? formValues[list]?.map((item, index) => (
                              <Draggable
                                key={item.id}
                                draggableId={item.id}
                                index={index}
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item md={12}>
                    ...
                    >
                      ...
                      {Object.values(INPUT_TYPES)?.map((col) => (
